...
def main(page: ft.Page):
    ...
    ml_lbl  = ft.Text("AI: connecting…",   size=10 if is_mobile() else 12, color=MUTED, max_lines=2, overflow=ft.TextOverflow.ELLIPSIS)
    ...
    ai_main = ft.Text("Waiting for AI…",  size=16 if is_mobile() else 18,
    ...
    def daily_ml_target():
        """Return whole-day AI feed/water target and source label."""
        try:
        ...
        # Main basis: AI prediction is already a one-day target.
        f = max(0.0, safe_float(ml.get("feedKg", 0), 0))
        w = max(0.0, safe_float(ml.get("waterL", 0), 0))
        if f > 0 or w > 0:
            return f, w, "AI daily prediction"
        ...
        for row in forecast or []:
            ...
            if row_date == today:
                ...
                if f_total > 0 or w_total > 0:
                    return f_total, w_total, "today's 7-day AI forecast"

        # Last fallback: sum available scheduled rows only when no daily AI target exists.
        ...
        if sched_rows:
            ...
            w_total = sum(max(0.0, safe_float(r.get("water_liters", r.get("waterL", 0)), 0)) for r in sched_rows)
            if f_total > 0 or w_total > 0:
                return f_total, w_total, "scheduled AI times"

        return 0.0, 0.0, "no AI daily target yet"
    ...
    def make_efficiency_graph(manual_feed, ml_feed, manual_water, ml_water, saved, wsaved, feed_eff, water_eff, source_label):
        ...
        if manual_feed >= 0:
            ...
            controls.extend([
                ...
                comparison_bar("AI daily feed target", ml_feed, feed_max, GREEN, "kg", bar_w),
                ft.Text(
                    f"Feed result: {fmt_kg(abs(saved))} " + ("saved" if saved >= 0 else "over the AI target"),
        ...
        if manual_water >= 0:
            ...
            controls.extend([
                ...
                comparison_bar("AI daily water target", ml_water, water_max, "#60a5fa", "L", bar_w),
                ft.Text(
                    f"Water result: {fmt_liters(abs(wsaved))} " + ("saved" if wsaved >= 0 else "over the AI target"),
    ...
    def compute_comparison(_=None):
        ...
        if ml_feed <= 0 and ml_water <= 0:
            cmp_note.value = "AI daily target is not ready yet."
            cmp_graph_holder.content = ft.Text("No AI daily target yet.", color=MUTED, size=12)
            su()
            return

        saved = manual_feed - ml_feed if manual_feed >= 0 else 0.0
        wsaved = manual_water - ml_water if manual_water >= 0 else 0.0

        feed_eff = None
        if manual_feed >= 0:
            if manual_feed > 0:
                feed_eff = max(-100.0, min(100.0, (saved / manual_feed) * 100.0))
            else:
                feed_eff = 0.0
        ...
        if manual_water >= 0:
            if manual_water > 0:
                water_eff = max(-100.0, min(100.0, (wsaved / manual_water) * 100.0))
            else:
                water_eff = 0.0
        ...
        while True:
            ...
            try:
                    ...
                    if esp:
                        if flow <= 0:
                            ...
                            if el >= 60:
                                ...
                                al_ic.color   = AMBER
                                al_mg.value   = f"⚠️ No water flow. Checking in {format_duration(r)}…"
                                al_mg.color   = AMBER
                        ...
                        al_ic.color   = AMBER
                        al_mg.value   = f"⚠️ ESP32 signal lost — last update was {format_duration(age)} ago"
                        al_mg.color   = AMBER
                        al_bx.bgcolor = SURF2
                ...
                if ml_stat:
                    ...
                    ml_lbl.value = {
                        "ready":      f"AI: ✅ {nr:,} records trained",
                        "training":   f"AI: 🔄 retraining {nr:,} records…",
                        "collecting": f"AI: collecting ({nr} records)…",
                        "error":      f"AI: ⚠️ {ml_stat.get('error','')[:40]}",
                        "waiting":    "AI: waiting for data…",
                    }.get(s, f"AI: {s}")
                ...
                if ml:
                    ...
                    ai_st.value   = (f"✅ Ready • {nr:,} records")
                    ai_st.color   = GREEN
                    ...
                    # AI prediction is a one-day target. The schedule must always
                    ...
                    # fills the missing times by splitting the daily AI target.
                    try:
                        pred_day = str(pd_)[:10]
                        fixed_schedule = [
                            (3,  "3:00 AM"),
                        ...
                        # The schedule is the one-day AI target divided by the
                    ...
                    ai_main.value   = "Waiting for AI predictions…"
                    ai_main.color   = MUTED
                ...
                # Chart image from AI server result only
                if ml:
    ...
    def objective_tab_page(*controls):
    ...
    objective_tabs = ft.Tabs(
        ...
        expand=True,
        tabs=[
            # Monitoring target: live sensor monitoring, AI status, and alert box
            ft.Tab(
                text="Monitoring",
                content=objective_tab_page(live_sec, ai_card, records_alert_sec),
            ),
            # Prediction target: AI prediction, ARIMA forecast, scheduled prediction, and 7-day forecast
            ft.Tab(
                text="Prediction",
                content=objective_tab_page(ai_pred_sec, arima_sec, fc7_sec),
            ),
            # Insights target: decision-support information, trends chart, and farm insights
            ft.Tab(
                text="Insights",
                content=objective_tab_page(chart_sec, ins_sec),
            ),
            # Compare target: manual vs ML comparison
            ft.Tab(
                text="Compare",
                content=objective_tab_page(compare_sec),
            ),
            # Records target: history and Firebase records
            ft.Tab(
                text="Records",
